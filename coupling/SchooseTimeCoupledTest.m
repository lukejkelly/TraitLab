function tests = SchooseTimeCoupledTest
...
function remainCoupledTests(testCase)
    ...
    while all(visited < 1e2)
        [i, newage_x, newage_y, logq_x, logq_y] ...
            = SchooseTimeCoupled(state, state);
...
function coupledTests(testCase)
    ...
    while all(visited < 5e3)
        [i, newage_x, newage_y, ~, ~] = SchooseTimeCoupled(state_x, state_y);
...
function overlap = getOverlap(state_x, state_y)
    ...
    for ind = 1:length(state_x.nodes)
        i = state_x.nodes(ind);
        [~, kT_x, jT_x, a_x, b_x] ...
            = SchooseTime.nodeTimesAndRanges(i, state_x);
        [~, kT_y, jT_y, a_y, b_y] ...
            = SchooseTime.nodeTimesAndRanges(i, state_y);
