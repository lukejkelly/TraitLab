...
% Last Modified by GUIDE v2.5 09-Jan-2007 22:49:12
GlobalSwitches;
GlobalValues;
...
if isequal(filename,0)|isequal(pathname,0)
    ...
    guidata(h,handles);
    disp(sprintf('%1.0f trees found in file\n',handles.numtrees));
...
if get(handles.cladescb,'Value')
    ...
    if numclade > (data.true.NS - 2)
        disp(sprintf('Number of clades must be less than or equal to number of internal nodes (%1.0f).\n No clades generated\n',data.true.NS-1));
    else
        disp(sprintf('Generating %1.0f clades\n',numclade));
...
% save it
disp(sprintf('Saving data to %s%s\n',handles.path, handles.file));
...
if ok
    ...
    if fid > 0
        ...
        if ~ok
            disp(sprintf('Encountered problem writing parameter file %s.\n Data file should be ok.',parfilename));
        end
    else
        disp(sprintf('\nProblem with opening %s%s\n  Data not saved',handles.path, handles.file));
    end
else
    disp(sprintf('\nSynthetic data not saved\n'));  
end
disp(sprintf('Data synthesizing complete\n'))

fig = pop('output');
draw(data.true.state.tree,fig.truefig,0,'Tree used for synthesising data');

%[x,y]=DepthDist(data.content,data.true.state);
%figure;plot(x,y,'.');xy=axis; hold on; plot([xy(1),xy(2)],[xy(1),xy(2)]); hold off;

 
exploredata(data.true.state,data.content,'',[],1,0,1);