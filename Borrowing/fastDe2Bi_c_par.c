/*
 * fastDe2Bi_c_par.c - used to convert decimal vectors to binary arrays.
 * Adapted from a similar function by Giovanni Motta.
 *
 * The calling syntax is:
 *
 *		P_b = fastDe2Bi_c_par(P_d, L),
 *
 * where
...
void fastDe2Bi_par(double *P_b, int M, double *L, double *P_d)
{
    ...
    for(i = 0; i < M; i++) /* Looping over rows of P_b/P_d. */
    {
        ...
        /* Looping over columns of P_b(i, :) to populate it. */
        for(j = 0; j < ((int) *L); j++)
        {
            /* We use bit-shifting and bitwise AND to convert decimal to
             * binary. */
